...
const COINBASE_AMOUNT = 1;
class UnspentTxOut {
    constructor(txOutId, txOutIndex, address, product, amount) {
...
exports.TxIn = TxIn;
class TxOut {
    constructor(address, product, amount) {
        this.address = address;
        this.product = product;
        this.amount = amount;
    }
...
const getTransactionId = (transaction) => {
    ...
    const txOutContent = transaction.txOuts
        .map((txOut) => txOut.address + txOut.product + txOut.amount)
        .reduce((a, b) => a + b, '');
...
const getCoinbaseTransaction = (address, blockIndex) => {
    ...
    t.txIns = [txIn];
    t.txOuts = [new TxOut(address, 'NA', COINBASE_AMOUNT)];
    t.id = getTransactionId(t);
    return t;
};
exports.getCoinbaseTransaction = getCoinbaseTransaction;
const getBaseTransaction = (address, product, amount, blockIndex) => {
    const t = new Transaction();
    const txIn = new TxIn();
    txIn.signature = '';
    txIn.txOutId = '';
    txIn.txOutIndex = blockIndex;
    t.txIns = [txIn];
    t.txOuts = [new TxOut(address, product, amount)];
    t.id = getTransactionId(t);
    return t;
};
exports.getBaseTransaction = getBaseTransaction;
...
const updateUnspentTxOuts = (aTransactions, aUnspentTxOuts) => {
    const newUnspentTxOuts = aTransactions
        .map((t) => {
        return t.txOuts.map((txOut, index) => new UnspentTxOut(t.id, index, txOut.address, txOut.product, txOut.amount));
    })
...
const isValidTxOutStructure = (txOut) => {
    ...
    else if (typeof txOut.product !== 'string'){
        console.log('invalid Product type in txOut');
        return false;
    }
