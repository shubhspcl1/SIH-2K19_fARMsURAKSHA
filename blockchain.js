...
const genesisTransaction = {
    ...
    'id': '51142cc47bbaed28929c63943012bd43897cba24190f85f4271da1955a75c02d'
};
...
const getTransactionMap = () => transactionMap;
exports.getTransactionMap = getTransactionMap;
...
const generateNextBlock = (product, amount) => {
    ...
    const tx = transaction_1.getBaseTransaction(wallet_1.getPublicFromWallet(), product, amount, getLatestBlock().index + 1);
    const blockData = [tx].concat(transactionPool_1.getTransactionPool());
    return generateRawNextBlock(blockData);
};
exports.generateNextBlock = generateNextBlock;
const generatenextBlockWithTransaction = (receiverAddress, product, amount) => {
    ...
    const tx = wallet_1.createTransaction(receiverAddress, product, amount, wallet_1.getPrivateFromWallet(), getUnspentTxOuts(), transactionPool_1.getTransactionPool());
...
const sendTransaction = (address, product, amount) => {
    const tx = wallet_1.createTransaction(address, product, amount, wallet_1.getPrivateFromWallet(), getUnspentTxOuts(), transactionPool_1.getTransactionPool());
...
const addBlockToChain = (newBlock) => {
    if (isValidNewBlock(newBlock, getLatestBlock())) {
        ...
        if (retVal === null) {
            ...
            if(newBlock.data.length > 1)
                transactionMap[newBlock.data[1].id] = newBlock;
            else
                transactionMap[newBlock.data[0].id] = newBlock;
            try{
                history[wallet_1.getPublicFromWallet()].push(newBlock);
            }
            catch(e){
                history[wallet_1.getPublicFromWallet()] = [newBlock];
            }        
            return true;
        }
    }
...
const queryTransactionHistory = (id) => {
    var history = [transactionMap[id]]
    var current = transactionMap[id]
    history.forEach(obj => {
        if(obj.data.length > 1)
        obj.data[1].txIns.forEach(txObj => {
            if(txObj.txOutIndex == 0)
                history.push(transactionMap[txObj.txOutId]);
        });
    });
    return history;
};
exports.queryTransactionHistory = queryTransactionHistory;
